...
use wendao_nexus_core::{
    ...
    ExternalKnowledgeSearchResponse, KnowledgeSourceConnector, ProvenanceBundle, SourceDomain,
...
pub(crate) struct FixtureFlightHarness {
    ...
    pub(crate) artifact_store: LocalFileArtifactStore,
...
impl FixtureFlightHarness {
    ...
    pub(crate) async fn handle_descriptor(
        &self,
    ...
    pub(crate) async fn handle_command_descriptor_result(
        &self,
        command: NexusFlightCommand,
    ) -> Result<arrow_array::RecordBatch, NexusFlightProviderError> {
